...
function mysite_print_cufon() {
	...
	foreach( $active_cufon['fonts'] as $declaration => $font ) {
		...
		if( $declaration == '.jqueryslidemenu a' )
		{
			...
			$out .= "\r\tCufon.replace('.jqueryslidemenu>ul>li>a', { fontFamily: '{$font}' });";
...
<ul class="sitemap_list"><?php wp_list_categories( array( 'exclude'=> mysite_exclude_category_string( $minus = false ), 'feed' => __( 'RSS', MYSITE_TEXTDOMAIN ), 'show_count' => true, 'use_desc_for_title' => false, 'title_li' => false ) );
?></ul>
<div class="divider top"><a href="#"><?php _e( 'Top', MYSITE_TEXTDOMAIN ); ?></a></div>
...
?><li><a href="<?php esc_url( the_permalink() ); ?>" rel="bookmark" title="<?php printf( __( "Permanent Link to %s", MYSITE_TEXTDOMAIN ), esc_attr( get_the_title() ) ); ?>"><?php the_title(); ?></a> (<?php comments_number('0', '1', '%'); ?>)</li>
<?php endwhile;
?></ul>
<div class="divider top"><a href="#"><?php _e( 'Top', MYSITE_TEXTDOMAIN ); ?></a></div>
